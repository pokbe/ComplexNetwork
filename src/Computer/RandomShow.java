...
public class RandomShow extends JFrame{
	...
	public RandomShow(RandomAttack ac,RandomAttack home,RandomAttack dia,int AorC){
		...
		if(AorC==1){
			...
			createTimeXYChar("APL of RandomAttack", "number of node reduce", "APL", datasetapl); 
		}else if(AorC==2){
			CategoryDataset datasetC = getBarData(Clustercollection, rowKeys, columnKeys);
			createTimeXYChar("Cluster of RandomAttack", "number of node reduce", "C", datasetC);
		}else{
			CategoryDataset datasetC = getBarData(SubSize, rowKeys, columnKeys);
			createTimeXYChar("SubSize of RandomAttack", "number of node reduce", "S", datasetC);
		}
	}
...
//	private XYDataset createDataset(double)
}
